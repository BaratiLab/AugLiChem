...
class Runner(object):
    ...
    def train(self):
        ...
        for epoch_counter in range(self.config['epochs']):
            for bn, (aug_data) in enumerate(train_loader):
                optimizer.zero_grad()

                aug_data = aug_data.to(self.device)
                loss = self._step(model, aug_data, n_iter)
        ...
        return valid_loss, valid_ame#loss, metric
    ...
    def _validate(self, model, valid_loader):
        ...
        if self.config['dataset']['task'] == 'regression':
            ...
            #print("pred_arr",predictions.shape)
    ...
    def _test(self, model, test_loader):
        ...
        with torch.no_grad():
            ...
            test_loss = 0.0
            num_data = 0
            for bn, (data, __) in enumerate(test_loader):
                data = data.to(self.device)
                pred = model(data)
                loss = self._step(model, data, bn)
